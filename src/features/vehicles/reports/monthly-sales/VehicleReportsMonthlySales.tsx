...
const VehicleReportsMonthlySales = () => {
    ...
    const {
        ...
        loading: getLoading,
        error: getError,
    } = useGetAPI<any[]>(`${API_BASE_URL}/api/vehicles/reports/monthly-sales`)
    ...
    useEffect(() => {
        ...
        selectedYearData.forEach((monthlyData: any) => {
            xAxisData.push(
                new Date(0, monthlyData.month).toLocaleString('default', {
                    month: 'short',
                })
            )
            seriesData.push(monthlyData.sales)
        })

        setXAxis(xAxisData)
        setSeries([{ data: seriesData, label: 'Sales', type: 'bar' }])
    ...
    return (
        <Container
            data-testid="monthly-sales-content"
            maxWidth="lg"
            sx={{ pt: 8, pb: 8 }}
        >
...
export default VehicleReportsMonthlySales
